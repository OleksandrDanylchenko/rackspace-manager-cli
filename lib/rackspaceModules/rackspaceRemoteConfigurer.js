    ...
    async getRemoteRecords(rackspacePath) {
        ...
        return new Promise((resolve) => {
            child_process_1.exec(`swiftly get ${containerName} --prefix ${containerFolderName}`, (error, stdout, stderr) => {
                if (error || stderr) {
                    throw new Error(`Sorry, but "swiftly" hasn't been found on your PC. Please install it, using the provided link and try again: https://docs.rackspace.com/support/how-to/install-the-swiftly-client-for-cloud-files/`);
                }
